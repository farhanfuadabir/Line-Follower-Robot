...
// 		if (exception != 0)
// 		{
// 			Serial_sendInt(exception_1_Count, DEC);
// 			Serial_sendString("\t \t");
// 			Serial_sendInt(exception_2_Count, DEC);
// 			Serial_sendString("\n");
// 		}
...
// 			inverseLevel = 0;
// 		}
// 		else if (inverseLevel > 50)
// 		{
// //  			Serial_sendString("OK\tIN \n \n");
// 			inverseFlag = 1;
// 			inverseLevel = 0;
// 			rightFlag = 1;
// 			reverseFlag = 1;
...
// 		{
// 			priorMappedValue = prev_mappedValue;
// 			for (char whiteConfidence = 0; whiteConfidence < 10; whiteConfidence++)
// 			{
// 				sensorValueConversion();
// 				sensorMapping();
// 				if (sensorValue != 0b00000000) break;
// 			}
// 			
// 			if (priorMappedValue >= 0)
// 			{
// 				Backward();
// 				_delay_ms(200);
// 				RightSharp();
...
// 			Serial_sendString("\t");
	...
	else if (inverseFlag == 1)
	{
		...
		{
			int temp = adcRead(sensorNumber);
			if (temp < compareValueBlackSurface[sensorNumber]) digitalValue = 0;
			else
			{
				digitalValue = 1;
				int8_t tempVal = ((sensorNumber * 2) - 7);
...
// 	Serial_sendInt(sensorValue, BIN);
// 	Serial_sendString("\t");
// 	Serial_sendInt(zero_count, DEC);
// 	Serial_sendString("\t");
// 	Serial_sendInt(weightDiv, DEC);
// 	Serial_sendString("\n");	
}
...
void PID_Correction(void) //my algo
{
	...
	{
		straightLineCount++;
		if (straightLineCount > 500) maxSpeed = 230;
	}
	else 
	{
		straightLineCount = 0;
...
// 	else if (exception == 2 && exception_2_Count > 3) // white e 8 silo /* && acuteFlag == 1*/)
// 	{
// 		sensorMapping();
...
// 			if (sensorValue == 0x00)
// 			{
// 				Backward();								// no backward @non white
// 				_delay_ms(50);
// 				Stop();
// 				_delay_ms(50);
// 				RightSharp();
...
// 			{
// 				Backward();
// 				_delay_ms(50);							// no backward @non white
// 				Stop();
// 				_delay_ms(50);
// 				LeftSharp();
// 				_delay_ms(200);
...
// 		}
// 	}
	else if (exception == 2 && exception_2_Count > 2/* && acuteFlag == 0*/)
	{
		if (weightedValue >= 0 && mappedValue == 120)
		{
			Stop();
